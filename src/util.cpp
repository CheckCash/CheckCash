...
const char * const BITCOIN_CONF_FILENAME = "checkcash.conf";
const char * const BITCOIN_PID_FILENAME = "checkcashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "checkcash" is a composite category enabling all CheckCash-related debug output
            if(ptrCategory->count(string("checkcash"))) {
...
#else
    const char* pszModule = "checkcash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CheckCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CheckCash
    // Mac: ~/Library/Application Support/CheckCash
    // Unix: ~/.checkcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CheckCash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CheckCash";
#else
    // Unix
    return pathRet / ".checkcash";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty checkcash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override checkcash.conf
